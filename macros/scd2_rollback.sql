/*
    scd2_rollback / scd2_restore_last_rollback
    ------------------------------------------
    Manual, operator-invoked rollback of a published SCD2 model to a prior
    run or date. Works entirely through the approval table, which is what
    makes it near-instant: un-approving a batch makes its rows invisible to
    the published view immediately (the EXISTS semi-join simply stops
    matching). No data is moved.
    ...
    Usage (dry run is the DEFAULT - it only reports):
      ...
      dbt run-operation scd2_rollback --args '{
          "model_name": "dim_entity__history",
    ...
    audit.scd2_rollback_log (who, when, criteria, original approved_at),
    ...
    *** DATA LOSS WARNING ***
    ...
    for rolling-window sources, extract dates older than the retention
    window are gone for good. If you roll back past staging retention and
    let the purge happen, those versions are permanently lost. Restore
    (or export the rows) BEFORE the next scheduled run if in doubt.
*/


{% macro scd2_bootstrap_rollback_log() %}
    {% if execute %}
        {% set sql %}
            IF NOT EXISTS (SELECT 1 FROM sys.schemas WHERE name = 'audit')
                EXEC('CREATE SCHEMA audit');

            IF OBJECT_ID('audit.scd2_rollback_log') IS NULL
                CREATE TABLE audit.scd2_rollback_log
                (
...
{% macro scd2_rollback(model_name, to_batch_id=none, to_datetime=none, dry_run=true, purge=false) %}
    {% if not execute %}{% do return(none) %}{% endif %}

    {#- exactly one anchor -#}
    {% if (to_batch_id is none) == (to_datetime is none) %}
        {{ exceptions.raise_compiler_error(
            "scd2_rollback: provide exactly one of to_batch_id or to_datetime.") }}
    {% endif %}

    {% do scd2_bootstrap_audit() %}
    {% do scd2_bootstrap_rollback_log() %}
    ...
    {% if to_batch_id is not none %}
        ...
        {% if res.rows | length == 0 %}
            {{ exceptions.raise_compiler_error(
                "scd2_rollback: batch_id '" ~ to_batch_id ~ "' was never approved for '" ~
                model_name ~ "'. Query audit.scd2_approved_batches for valid anchors.") }}
        {% endif %}
    ...
    {% set affected = run_query(
        ...
        "from audit.scd2_approved_batches a " ~
    ...
    {% if affected.rows | length == 0 %}
        {% do log('scd2_rollback: nothing to roll back - no ' ~ model_name ~
                  ' batches approved after ' ~ cutoff, info=True) %}
        {% do return(none) %}
    {% endif %}

    {% do log('scd2_rollback: ' ~ ('DRY RUN - ' if dry_run else '') ~ 'rolling back ' ~
    ...
    {% if dry_run %}
        {% do log('scd2_rollback: dry run only - nothing changed. ' ~
    ...
    {% do run_query(
        "insert into audit.scd2_rollback_log " ~
    ...
    {% do log('scd2_rollback: approval rows deleted - ' ~ model_name ~
              ' published view now reflects state as of ' ~ cutoff, info=True) %}

    {#- 3. optional physical purge (same statement the pre-hook runs) -#}
    {% if purge %}
        {% do run_query(scd2_purge_unapproved(hist)) %}
        {% do log('scd2_rollback: un-approved rows physically purged from ' ~ hist ~
                  ' - this rollback is now IRREVERSIBLE from the warehouse.', info=True) %}
    {% else %}
        {% do log('scd2_rollback: physical rows retained - reversible via ' ~
                  'scd2_restore_last_rollback UNTIL the next scheduled run purges them. ' ~
                  'If the rolled-back extract dates exceed staging retention, restore or ' ~
                  'export before that run or the versions are permanently lost.', info=True) %}
    {% endif %}

    {#- approved set changed -> rebuild + swap the consumption table -#}
    {% set marker = scd2_marker_for_model(model_name) %}
    {% if marker %}
        ...
        {% if refresh is not mapping or not refresh.get('ok') %}
            ...
            {{ scd2_fail_run(
                'scd2_rollback: consumption table refresh failed for ' ~ marker ~ '. ' ~ err) }}
        {% endif %}
    {% endif %}
...
{% macro scd2_restore_last_rollback(model_name, rollback_id=none) %}
    {% if not execute %}{% do return(none) %}{% endif %}

    {#- default to the most recent rollback event for this model -#}
    {% if rollback_id is none %}
        {% set res = run_query(
            "select top 1 rollback_id from audit.scd2_rollback_log " ~
            "where model_name = '" ~ model_name ~ "' order by executed_at desc") %}
        {% if res.rows | length == 0 %}
            {{ exceptions.raise_compiler_error(
                "scd2_restore_last_rollback: no rollback events logged for '" ~ model_name ~ "'.") }}
        {% endif %}
    ...
    {% do run_query(
        ...
        "from audit.scd2_rollback_log l " ~
        "where l.rollback_id = '" ~ rollback_id ~ "' and l.model_name = '" ~ model_name ~ "' " ~
        "and not exists (select 1 from audit.scd2_approved_batches a " ~
    ...
    {% set orphaned = run_query(
        "select l.batch_id from audit.scd2_rollback_log l " ~
        "where l.rollback_id = '" ~ rollback_id ~ "' and l.model_name = '" ~ model_name ~ "' " ~
        "and not exists (select 1 from " ~ hist ~ " h where h._batch_id = l.batch_id)") %}

    {% if orphaned.rows | length > 0 %}
        {% do log('scd2_restore_last_rollback: WARNING - ' ~ (orphaned.rows | length) ~
    ...
    {% else %}
        {% do log('scd2_restore_last_rollback: rollback ' ~ rollback_id ~ ' fully reversed - ' ~
    ...
    {% if marker %}
        ...
        {% if refresh is not mapping or not refresh.get('ok') %}
            {% set err = refresh.get('error', 'unknown refresh failure') if refresh is mapping else 'unknown refresh failure' %}
            {{ scd2_fail_run(
                'scd2_restore_last_rollback: consumption table refresh failed for ' ~ marker ~ '. ' ~ err) }}
        {% endif %}
    {% endif %}
